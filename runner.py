...
class FidesysRunner:
    ...
    def calculation_static(self, result_script):
    ...
    def calculation_buckling(self, result_script):
        output_pvd_path = os.path.join(self.base_dir + "\\" + f"1.pvd")  # Объявляем директорию и файл сохранения
        for line in result_script:
            print(line, "cmd")
            self.fidesys.cmd(line)
        self.fidesys.cmd("analysis type buckling elasticity dim3")
        self.fidesys.cmd("eigenvalue find 1 smallest")
        self.fidesys.cmd("calculation start path '" + output_pvd_path + "'")