...
class Bayesian_optimization():
    ...
    def random_search(self, number_of_points, range_of_values, context, sim_result):
        ...
        for _ in range(number_of_points):
            ...
            #     ) + 1)
            # )
            penalty = np.sin(3 * new_params["h"]) + 0.5 * np.sin(8 * new_params["h"]) + new_params["h"] ** 2 / 10
    ...
    def func(self, x_vec, param_names, context, sim_result):
        ...
        penalty=np.sin(3*x_vec) + 0.5*np.sin(8*x_vec) + x_vec**2 / 10
    ...
    def optimize(self, context, progress_queue):
        ...
        X_train, Y_train = self.hyperparams_random_search(5, range_of_values, context, sim_result, 100, [3, 15],
                                                          [0.05, 1])
        ...
        # Пересчитываем GP на всех собранных точках
        X_train_final = X[visited_idx]
        y_train_arr = np.array(y_train)
        y_mean = np.mean(y_train_arr)
        y_std = np.std(y_train_arr)
        if y_std < 1e-12:
        ...
        data = [y_train_norm, X_train_final]
        mu = self.baesian(data, X)
        sigma = self.distributions(data, X)
        mu = y_mean + y_std * mu
        sigma = y_std * sigma
